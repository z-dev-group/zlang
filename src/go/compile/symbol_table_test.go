...
func TestResolveGlobal(t *testing.T) {
	...
	for _, sym := range expected {
		...
		if result != sym {
			t.Errorf("expected %s to resole to %+v, got=%+v", sym.Name, sym, result)
		}
	}
}
...
func TestResolveNestedLocal(t *testing.T) {
	...
	for _, tt := range tests {
		for _, sym := range tt.expectedSymols {
...
func TestDefineResolveBuiltins(t *testing.T) {
	...
	for _, table := range []*SymbolTable{global, firstLocal, secondLocal} {
		for _, sym := range expected {
			...
			if !ok {
				t.Errorf("name %s not resovled", sym.Name)
			}
			if result != sym {
				t.Errorf("expected %s to resolve to %+v, got=%+v", sym.Name, sym, result)
			}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	for _, sym := range expected {
		...
		if !ok {
			t.Errorf("name %s to resolve to %+v, got=%+v", sym.Name, sym, result)
		}
	}
...
func TestDefineAndResolveFunctionName(t *testing.T) {
	...
	if !ok {
		t.Fatalf("function name %s not resolve to %+v, got=%+v", expected.Name, expected, result)
	}
}
...
func TestShadowingFunctionName(t *testing.T) {
	...
	if result != expected {
		t.Errorf("expected %s resovle to %+v,got=%+v", expected.Name, expected, result)
	}
}
