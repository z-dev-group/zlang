...
func NewWithGlobalsStore(bytecode *compile.Bytecode, s []object.Object) *VM {
	vm := New(bytecode)
	vm.globals = s
	return vm
}
...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpIndex:
			...
			err := vm.executeIndexExpress(left, index)
			if err != nil {
				return err
			}
		case code.OpGetBuiltin:
			...
			vm.currentFrame().ip += 1
			defination := object.Builtins[builtinIndex]
			err := vm.push(defination.Builtin)
			if err != nil {
				return err
			}
		case code.OpClosure:
...
func (vm *VM) executeIndexExpress(left, index object.Object) error {
	switch {
...
func (vm *VM) executeCall(numArgs int) error {
	...
	case *object.Builtin:
		return vm.callBulitin(callee, numArgs)
	default:
		return fmt.Errorf("calling non-function and non-built-in")
	}
}

func (vm *VM) callBulitin(builtin *object.Builtin, numArgs int) error {
