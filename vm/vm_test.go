...
func TestFucntionWithoutReturnValue(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
			let noReturn = fn() {};
...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case map[object.HashKey]int64:
		...
		if len(hash.Pairs) != len(expected) {
			t.Errorf("hash has wrong number of pairs. wnat=%d, got=%d", len(expected), len(hash.Pairs))
			return
		}
...
func testIntegerObject(expected int64, actual object.Object) error {
	...
	//return fmt.Errorf("object is not integer. got=%T (%+v)", result, result)
	if !ok {
...
func TestRecursiveFibonalcci(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
			let fibonacci = fn(x) {
